...
from fastapi import FastAPI, Request, Form, HTTPException, Depends, status, Path, Body
    ...
    try:
        ...
        if user_doc.exists:
            ...
            return {
                ...
                "role": user_data.get("role", "user")  # Defaults to "user"
            }
        else:
            # If user doc doesn't exist, treat them as a basic user
            return {
                "id": user_id,
...
async def register_user(
    ...
    try:
        ...
        claims = google.oauth2.id_token.verify_firebase_token(token, firebase_request_adapter)
        user_id = claims["user_id"]
        ...
        if user_ref.get().exists:
            return JSONResponse(content={"message": "User already registered."}, status_code=400)
...
async def index_or_main(request: Request):
    ...
    if id_token:
        try:
            ...
            if user:
                ...
                # If user is authenticated, go to main.html
    ...
    # If no token or invalid, show index.html
...
async def edit_task(
    ...
    # Remove red highlight flag if task is now assigned
    if assigned_to:
        update_data["unassigned_due_to_removal"] = firestore.DELETE_FIELD
...
# Get task counts (active, completed, total)
@app.get("/task-board/{board_id}/task-counts")
async def get_task_counts_route(board_id: str):
    counts = get_task_counts(board_id)
    return {"counts": counts}


# Helper function to get task counts
def get_task_counts(board_id: str):
    tasks_ref = firestore_db.collection("tasks").where("board_id", "==", board_id)
    all_tasks = list(tasks_ref.stream())
    total = len(all_tasks)
    completed = sum(1 for task in all_tasks if task.to_dict().get("completed"))
    active = total - completed
    return {"total": total, "completed": completed, "active": active}


# Display assigned tasks for the user
@app.get("/task-board/{board_id}")
async def show_board(request: Request, board_id: str):
    id_token = request.cookies.get("token")
    user = get_user_from_token(id_token) if id_token else None
    ...
    # Only show tasks assigned to the current user unless they are the creator
    tasks_ref = firestore_db.collection("tasks").where("board_id", "==", board_id).stream()
    tasks = []
    for task in tasks_ref:
        task_data = task.to_dict()
        if user["id"] == board_data["created_by"] or task_data.get("assigned_to") == user["id"]:
